    ...
    <div id="page-content-wrapper">
        ...
        <section class="page-section" id="predict">
            <div class="container" style="text-align: center">
                <h2 class="text-center mt-0">全球确诊人数预测</h2>
...
<script>
    ...
    <c:forEach items="${predictList}" var="predict">
        ...
        <c:choose>
        <c:when test="${predict.getGlobal_confirm()==0}">
            a.push('');
        </c:when>
        <c:otherwise>
            a.push(${predict.getGlobal_confirm()});
        </c:otherwise>
        </c:choose>
        <c:choose>
        <c:when test="${predict.getGlobal_confirm_predict()==0}">
            predict.push('');
        </c:when>
        <c:otherwise>
            predict.push(${predict.getGlobal_confirm_predict()});
        </c:otherwise>
        </c:choose>
        <c:choose>
        <c:when test="${predict.getGlobal_confirm_adjust()==0}">
            adjust.push('');
        </c:when>
        <c:otherwise>
            adjust.push(${predict.getGlobal_confirm_adjust()});
        </c:otherwise>
        </c:choose>
    </c:forEach>
    option = {
